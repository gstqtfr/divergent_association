...
\begin{abstract}
An analysis of the concept of divergent association and its relationship to creativity. The divergent association test (DAT) asks the participant to generate a list of ten \emph{unrelated} words. The distance between the words on the list are then measured to find their semantic distance. The suggestion behind the test is that creative people are capable of listing words with greater semantic distance, which could in turn serve as an objective measure of divergent thinking. Taking this test of divergence and creativity, we apply the test to large language models (LLMs), measure the semantic distance using the same methodology, and compare with human performance on the test. Can LLMs perform with the same, or perhaps greater, divergence than humans?
\end{abstract}

%----------------------------------------------------------

\keywords{divergent association, DAT, large language models, LLMs, creativity}
...
\begin{figure}
  \centering
  \includegraphics[scale=0.75]{cos_dist_llama3_3.eps}
  %\includegraphics[width=90mm]{cos_dist_llama3_3.jpg}
